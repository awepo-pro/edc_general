...
def split_into_sentences(text: str) -> list[str]:
    ...
    sentences_list = [s.strip() for s in sentences if s.strip()]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='Use Chinese prompt (stored in ./prompt_templates_zh/)'
    )
    ...
    if args['zh']:
        print('using Chinese template')
        for arg_key in args.keys():
            if 'prompt_template_file_path' in arg_key:
                args[arg_key] = f'./{Path(args[arg_key]).parent}_zh/{Path(args[arg_key]).name}'
    ...
    input_text: str = open(args["input_text_file_path"], "r").read()
    input_text_list: list[str] = split_into_sentences(input_text)
